...
farmTitle.Text = "AUTO FARM • NPC LEVEL"
...
local function getPlayerLevel()
	...
	for _,obj in ipairs(char:GetDescendants()) do
		if obj:IsA("TextLabel") or obj:IsA("TextBox") then
			...
			local value = tonumber(string.match(text,"%[Lv%.?%s*(%d+)%]"))
				or tonumber(string.match(text,"%[Lvl%.?%s*(%d+)%]"))
				or tonumber(string.match(text,"%[Level%s*(%d+)%]"))
			if value then
				return value
			end
		end
...
local function getMobLevel(mob)
	...
	local directLevel = getNumber(mob,{
		"Level",
		"Lvl",
		"Lv",
		"EnemyLevel",
	...
	for _,obj in ipairs(mob:GetDescendants()) do
		if obj:IsA("TextLabel") or obj:IsA("TextBox") then
			local text = obj.Text or ""
			local value =
				tonumber(string.match(text,"%[Lv%.?%s*(%d+)%]"))
				or tonumber(string.match(text,"%[Lvl%.?%s*(%d+)%]"))
				or tonumber(string.match(text,"%[Level%s*(%d+)%]"))
			if value then
				return value
...
local function findBestMobByLevel()
	local playerLevel = getPlayerLevel()
	if not playerLevel then
		return nil,nil,nil,"Không đọc được Level của player"
	end
	...
	if not root then
		return nil,nil,nil,"Không tìm thấy HumanoidRootPart"
	end
	for _,mob in ipairs(enemies:GetDescendants()) do
		if mob:IsA("Model") and mob ~= char then
			...
			if humanoid and mobRoot and humanoid.Health > 0 then
				...
				if mobLevel and mobLevel <= playerLevel then
					local distance = (mobRoot.Position - root.Position).Magnitude
					if distance <= farmRange then
						if mobLevel > bestLevel
							or (mobLevel == bestLevel and distance < bestDistance) then
							bestMob = mob
							bestLevel = mobLevel
	...
	if not bestMob then
		return nil,nil,nil,
			"Không tìm thấy NPC có Lv <= " .. tostring(playerLevel)
	end
	return bestMob,bestLevel,playerLevel,"OK"
end
...
local function farmStep()
	...
	farmBusy = true
	local mob,mobLevel,playerLevel,status = findBestMobByLevel()
	if not mob then
		currentMob = nil
		farmStatus.Text = "Status: " .. tostring(status)
		farmBusy = false
		return
	end
	if currentMob
		and currentMob.Parent
		and currentMob ~= mob then
		local oldHumanoid = currentMob:FindFirstChildOfClass("Humanoid")
		if oldHumanoid and oldHumanoid.Health > 0 then
			mob = currentMob
			mobLevel = getMobLevel(currentMob) or mobLevel
		else
			currentMob = nil
		end
	end
	...
	local mobRoot = currentMob:FindFirstChild("HumanoidRootPart")
		or currentMob.PrimaryPart
	if root and humanoid and mobRoot and humanoid.Health > 0 then
		root.CFrame = mobRoot.CFrame * CFrame.new(0,farmHeight,0)
		farmStatus.Text =
			"Status: FARMING" ..
			"\nPlayer Lv: " .. tostring(playerLevel) ..
			"\nTarget: " .. currentMob.Name ..
			" [Lv. " .. tostring(mobLevel) .. "]" ..
