...
local function updBtn(btn,on)
	if on then
		btn.Text=btn.Tag.." ON"
		btn.BackgroundColor3=Color3.fromRGB(55,155,90)
	else
		btn.Text=btn.Tag.." OFF"
		btn.BackgroundColor3=Color3.fromRGB(90,90,100)
	end
end

local function getFarm()
	return workspace:FindFirstChild("NPCSpawner") or workspace:FindFirstChild("Enemies") or workspace
end
...
local function flyLoop()
	...
	fc=run.RenderStepped:Connect(function()
		...
		if mov.Magnitude>0 then
			bv.Velocity=mov.Unit*cfg.flySpeed
		else
...
local function clearESP()
	if espFolder then
		for _,o in ipairs(espFolder:GetChildren())do pcall(function()o:Destroy()end)end
	end
end
...
local function mkGUI()
	...
	topCrn.Parent=top	
	mkLbl("VANH • BLOX FRUIT",UDim2.new(1,-100,1,0),UDim2.new(0,18,0,0),top,Color3.fromRGB(255,255,255),20)	
	...
	con(opn.InputBegan,function(inp)
		if inp.UserInputType==Enum.UserInputType.MouseButton1 then
			dragO=true
			dragOS=inp.Position
			posO=opn.Position
	...
	con(opn.InputEnded,function(inp)
		if inp.UserInputType==Enum.UserInputType.MouseButton1 then dragO=false end
	end)	
	con(uis.InputChanged,function(inp)
		if dragO and inp.UserInputType==Enum.UserInputType.MouseMovement then
			local dlt=inp.Position-dragOS
